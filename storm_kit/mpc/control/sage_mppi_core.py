...
"""
Standalone clean SAGE controller core.

This controller intentionally does not inherit from the legacy `sage_mppi.py`
implementation. It is the paper-aligned clean path for future ablation and
controller-only evaluation.

Design goals:
- keep the same external task/control-process interface used by STORM
- prefer rollout-native `delta_safe` / `safety_margin_seq`
- keep a backward-compatible fallback for old rollouts without native safety
- expose the three SAGE core ideas as explicit switches
"""
...
class SAGE_MPPI_CORE:
    """
    Independent clean SAGE controller.

    This class is intended to become the clean, analysis-ready controller core.
    It does not inherit from the legacy `sage_mppi.py`.
    """
    ...
    def _build_rollout_dict(
        ...
        if isinstance(rollout, dict):
            ...
            if self.prefer_rollout_native_margin and native_margin_ready:
                return rollout
            if "state_dict" in rollout and "costs" in rollout:
                return rollout

        # Backward-compatible path for old rollouts that do not emit native
        # safety outputs or state_dict.
        if hasattr(self.rollout_fn, "dynamics_model") and hasattr(self.rollout_fn, "cost_fn"):
            with profiler.record_function("sage_core/rollout/model"):
                state_dict = self.rollout_fn.dynamics_model.rollout_open_loop(state, act_seq)
            with profiler.record_function("sage_core/rollout/cost"):
    ...
    def _compute_rollout_safety_margin(self, rollout_dict: Dict[str, torch.Tensor]) -> torch.Tensor:
        # Paper-aligned path: native safety margins emitted directly by rollout.
        ...
        # Backward-compatible fallback: reconstruct from rollout state tensors.
        state_dict = rollout_dict.get("state_dict")
        if state_dict is None:
            self._used_margin_fallback = True
            return -torch.ones(rollout_dict["actions"].shape[0], **self.tensor_args)
        ...
        if hasattr(self.rollout_fn, "robot_self_collision_cost") and "state_seq" in state_dict:
            ...
            raw_signed_dist = self_cost.coll.check_self_collisions_nn(q_flat).view(batch_size, horizon)
    ...
    def _compute_safe_elite_covariance(
        ...
        self._last_safe_elite_fraction = float(safe_mask.to(torch.float32).mean().item())

        if not self.enable_safe_elite_shape:
    ...
    def _shift(self, shift_steps: int = 1):
        ...
        if self.base_action == "random":
            ...
            raise NotImplementedError(f"Unsupported base_action for SAGE core shift: {self.base_action}")
        ...
        with torch.amp.autocast("cuda", enabled=True):
            with torch.no_grad():
                for iter_idx in range(n_total):
                    ...
                    with profiler.record_function("sage_core_update"):
