...
from .sage_arm_base import SageArmBase
...
class SageArmReacher(SageArmBase):
    ...
    def cost_fn(
        ...
        cost = super().cost_fn(state_dict, action_batch, no_coll, horizon_cost)
        ...
        if self.exp_params["cost"]["joint_l2"]["weight"] > 0.0 and self.goal_state is not None:
            disp_vec = state_batch[:, :, 0 : self.n_dofs] - self.goal_state[:, 0 : self.n_dofs]
