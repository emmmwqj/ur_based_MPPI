...
WORLD_FILE="${SIM_DIR}/config/collision_world_dynamic_ball.yml"
...
CONTROLLER_PATTERN="python3 scripts/reach_dynamic_ball.py"
...
log() {
    echo "[run_all_reach_dynamic_ball] $*" >&2
}
...
echo "============================================================"
echo "UR7e Dynamic Primitive Ball Reaching - Unified Launcher"
echo "============================================================"
...
log "starting dynamic reaching controller"
setsid bash -lc 'cd "$1" && shift && ./run_reach_dynamic_ball.sh "$@"' _ "${SIM_DIR}" "${CONTROLLER_ARGS[@]}" &
CONTROLLER_PGID=$!
