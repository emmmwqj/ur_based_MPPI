...
DEFAULT_OUTPUT_ROOT = Path("/tmp/sage_sequential_retargeting")

TARGET_SEQUENCE = [
    {"name": "p0_default", "x": 0.4, "y": -0.5, "z": 0.4, "publish": False},
    {"name": "p1", "x": 0.4, "y": -0.5, "z": 0.4, "publish": True},
    {"name": "p2", "x": 0.4, "y": 0.6, "z": 0.3, "publish": True},
    {"name": "p3", "x": 0.6, "y": 0.0, "z": 0.32, "publish": True},
    {"name": "p4", "x": 0.5, "y": -0.52, "z": 0.43, "publish": True},
    {"name": "p5", "x": 0.4, "y": -0.1, "z": 0.1, "publish": True},
]
...
def _cleanup_existing() -> None:
    ...
    _kill_matching("/tmp/sage_sequential_retargeting/.*/run_reach_static_tall.sh --no-rviz --max-steps")
    time.sleep(2.0)
...
def _spawn_controller(run_log: Path, max_steps: int) -> subprocess.Popen:
    command = f"cd {json.dumps(str(SCRIPT_DIR))} && ./run_reach_static_tall.sh --no-rviz --max-steps {max_steps}"
...
def _run_sequence(output_root: Path, max_steps: int, segment_timeout_s: float, stable_hits_required: int) -> dict:
    ...
    try:
        ...
        ctrl_proc = _spawn_controller(run_log, max_steps=max_steps)
        try:
...
def main() -> int:
    ...
    summary_file = args.output_root.resolve() / "summary.json"
    summary_file.write_text(json.dumps(result, indent=2, ensure_ascii=False))
    print(json.dumps(result, indent=2, ensure_ascii=False))
    return 0 if "target_validation_issues" not in result else 1
