...
# -*- coding: utf-8 -*-
"""
UR7e STORM MPC Control - Whole Gazebo ESDF 驱动示例
"""
...
def mpc_control_main(args):
    ...
    try:
        _log('=' * 60)
        _log('UR7e STORM MPC Control - Whole Gazebo ESDF')
        _log('=' * 60)

        config_dir = os.path.join(os.path.dirname(__file__), 'config')
        robot_file = os.path.join(config_dir, 'ur7e_robot_gazebo.yml')
        task_file = os.path.join(config_dir, 'ur7e_reacher_whole_gazebo.yml')
        world_file = os.path.join(config_dir, 'esdf_world_gazebo.yml')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='UR7e STORM MPC Whole Gazebo ESDF Control')
